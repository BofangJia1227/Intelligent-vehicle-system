...
short gyro[3], accel[3], sensors;//第一个数组用来存储陀螺仪的数据，第二个数组用来存储加速度计的原始数据
//零点漂移计数
int Deviation_Count;
...
//在一个定时器内读取MPU6050的陀螺仪和加速度计数据，并保存在数组中
    ...
    {	
      ...
      //开机前，读取陀螺仪零点			
		  if(Deviation_Count<CONTROL_DELAY)
				//Deviation_Count 和 CONTROL_DELAY用于计算陀螺仪和加速度计校准（零点漂移估计）时采集的样本数量。
				//在传感器上电后的前几秒钟内，代码会采集陀螺仪和加速度计的样本读数。Deviation_Count 用于计算已经采集的样本数。
				//Deviation_Count 的值从零开始，每次获取新的陀螺仪和加速度计读数样本时递增。一旦 Deviation_Count 达到一定值（由 CONTROL_DELAY 控制）
				//校准过程就被认为完成，此后 Deviation_Count 将不再用于校准。
				//该变量确定校准过程应持续的时间（以样本数来衡量）。
				//使用：CONTROL_DELAY 用于设置采集样本进行校准的样本数量。代码会一直采集样本，直到 Deviation_Count 达到这个值。
				//每次任务运行时，Deviation_Count 会递增，并且当它达到 CONTROL_DELAY 定义的值时，校准过程被认为完成。
				//校准过程中收集的样本用于估计陀螺仪和加速度计读数的零点漂移（偏移），然后从后续的传感器读数中减去这些偏移，以补偿零点漂移误差。
		  {	 
		  	Deviation_Count++;
			  memcpy(Deviation_gyro,gyro,sizeof(gyro));		//数据拷贝
				memcpy(Deviation_accel,accel,sizeof(accel));	//数据拷贝
		  }		
...
int16_t  MPU6050_FIFO[6][11];//缓冲区，用于声明为FIFO缓冲区，存储相关的数据，其中每行代表一个传感器数据的维度（例如，陀螺仪和加速度计）。
...
返回  值：无
**************************************************************************/
//函数通过循环操作将FIFO数组中的旧数据向前移动一个位置，为新数据腾出位置。这样，数组的前9个元素将被覆盖，第10个元素会被新的数据填充。
//接着，使用循环分别对 x、y、z 轴的加速度数据和角加速度数据进行累加，求得它们的和 sum。
//对于每个轴，将求得的和 sum 除以10，计算出一个平均值，并将该平均值存储到FIFO数组的第11个元素，即每个轴的最后一个位置。
//平滑传感器数据，减少噪声和抖动。在实际应用中，这有助于提高数据的稳定性和可靠性

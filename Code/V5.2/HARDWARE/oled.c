...
void OLED_ShowFloat(u8 x, u8 y, float num, u8 size) {
    ...
    u32 decimalPart = (u32)((num - integerPart) * 100 + 0.5); // 计算并四舍五入小数部分
	
    // 显示整数部分
    OLED_ShowNumber(x, y, integerPart, 2, size);
	
    // 计算显示小数点的位置
    x += (integerPart >= 10) ? size : size / 2;
    ...
    // 显示小数部分的第一位数字
    OLED_ShowChar(x, y, (decimalPart / 10) + '0', size, 1); // 显示小数部分的第一位数字
    x += size / 2; // 后移，准备显示第二个数字

    // 显示小数部分的第二位数字
    OLED_ShowChar(x, y, (decimalPart % 10) + '0', size, 1); // 显示小数部分的第二位数字
}

void OLED_ShowFloat_1(u8 x, u8 y, float num, u8 size) {
    u32 integerPart = (u32)num; // 获取整数部分
    u32 decimalPart = (u32)((num - integerPart) * 100 + 0.5); // 计算并四舍五入小数部分
    // 显示整数部分
    OLED_ShowNumber(x, y, integerPart, 2, size);
